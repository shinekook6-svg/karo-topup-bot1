...
// Command ဝင်လာရင် State ရှင်းပေးမယ့် Middleware
...
bot.command("start", async (ctx) => {
  ...
  await ctx.reply(`👋 မင်္ဂလာပါ ${fullName}!\nKaro TopUp Bot မှ ကြိုဆိုပါတယ်!။ အောက်ပါ ခလုတ်များကို နှိပ်ပြီး အသုံးပြုပါ။`, {
    reply_markup: getMainMenu(userId),
  });
});
//State တွေကိုစောင့်ပြီး စစ်မယ်
...
// --- ၃။ Admin ပို့လိုက်တဲ့ စာသားကို ဖမ်းမယ်---
...
// --- ၄။ Screenshot (Photo) ဖမ်းပြီး Confirm ခိုင်းမယ် ---
...
// --- ၄။ အတည်ပြုလိုက်ရင် D1 ထဲ သိမ်းပြီ ---
...
// --- TopUp ကို အတည်ပြုပြီး DB ထဲ သိမ်းခြင်း 
bot.callbackQuery("confirm_topup", async (ctx) => {
  ...
  // ပိုက်ဆံ ထပ်စစ်မယ် (Double Check)
  if (user.balance < item.price) {
...
`).bind(userId, item.id, item.item_name, playerId).run();
  ...
  await ctx.api.sendMessage(ADMIN_ID, `🔔 <b>TopUp Order အသစ် ရောက်ရှိ!</b>\n\n🆔 Order ID: #${orderId}\n💎 Item: ${item.item_name}\n🆔 Player ID: <code>${playerId}</code>\n👤 User: ${ctx.from.first_name}`);
});
...
bot.callbackQuery("topup_hist", async (ctx) => {
  ...
  let msg = "📜 <b>သင်၏ နောက်ဆုံးဝယ်ယူမှု ၅ ခု</b>\n\n";
  orders.results.forEach(o => {
    const statusEmoji = o.status === 'done' ? '✅' : (o.status === 'pending' ? '⏳' : '❌');
    msg += `${statusEmoji} #${o.id} | ${o.item_details}\n💰 ${o.status.toUpperCase()}\n\n`;
  });
